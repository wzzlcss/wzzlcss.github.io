...
<p style="text-align:justify;">
For ridge regression, the SAGA step size $\eta$ proposed by Defazio [@defazio2014] is defined as $\eta = \frac{1}{3(\mu n + L_{\text{max}})}$, where $L_{\text{max}}$ is bound by the largest sample-wise squared norm of the feature matrix. The optimal $\eta$ and batch size $\mathcal{B}$ for SAGA proposed by Gazagnadou are:

$$\begin{align}
\mathcal{B}^*  &=  \left \lfloor 1 + \frac{\mu (n-1)}{4 (\overline{L} + \lambda)} \right \rfloor \\
\eta^* &=  \frac{1}{4} \frac{1}{\text{max} \Big \{ \mathcal{L}(b) + \lambda, \frac{1}{b} \frac{n-b}{n-1} L_{\text{max}} + \frac{\mu n}{4 b} \Big \}} \\
